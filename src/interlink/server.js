  ...
  if (envelope.type === 'command') {
    ...
    if (cmd === 'override') {
      ...
      if (action === 'activate' && userId) {
        ...
        return { status: 200, body: { status: ok ? 'ok' : 'error', message: ok ? 'Override activated' : 'Failed to activate override' } };
      }
      if (action === 'deactivate' && userId) {
        const ok = deps.override.deactivate(userId);
        return { status: 200, body: { status: ok ? 'ok' : 'error', message: ok ? 'Override deactivated' : 'Override was not active' } };
      }
...
export function createInterlinkServer(deps) {
